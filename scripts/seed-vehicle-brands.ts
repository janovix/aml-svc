...
async function seedVehicleBrands(db: D1Database) {
	const adapter = new PrismaD1(db);
	const prisma = new PrismaClient({ adapter });

	try {
		console.log("Starting vehicle-brands catalog seeding...");
		...
		console.log(
			`Successfully seeded ${createdItems.count} vehicle brand(s) into the catalog.`,
		);
