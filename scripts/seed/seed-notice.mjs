 ...
 * Seed Notices
 *
 * Generates synthetic notice data for dev/preview environments.
...
async function seedNotices() {
	...
	try {
		console.log(`🌱 Seeding notices (${isRemote ? "remote" : "local"})...`);

		// Check if notices already exist
		const checkSql = "SELECT COUNT(*) as count FROM notices;";
		...
		try {
			...
			if (countMatch && parseInt(countMatch[1], 10) > 0) {
				console.log(`⏭️  Notices already exist. Skipping seed.`);
				return;
			}
		} catch {
			// If check fails, continue with seeding
			console.warn(
				"⚠️  Could not check existing notices, proceeding with seed...",
			);
		} finally {
			try {
		...
		// TODO: Implement notice seeding logic
		// Generate synthetic notices with realistic data for testing
		// Note: Notices are SAT XML notices that contain alerts
		// For now, skip if no notices exist
		console.log("✅ Notice seeding completed (no synthetic data generated)");
	} catch (error) {
