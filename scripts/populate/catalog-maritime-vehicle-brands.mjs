#!/usr/bin/env node
/**
 * Populate Maritime Vehicle Brands Catalog
 *
 * This script populates the maritime-vehicle-brands catalog with common boat/yacht brands.
...
const sqlFile = join(__dirname, "catalog-maritime-vehicle-brands.sql");
...
try {
	console.log(
		`📦 Populating maritime-vehicle-brands catalog (${isRemote ? "remote" : "local"})...`,
	);
	...
	console.log("✅ Maritime vehicle brands catalog populated successfully!");
} catch (error) {
	console.error("❌ Error populating maritime-vehicle-brands catalog:", error);
	process.exit(1);
}
