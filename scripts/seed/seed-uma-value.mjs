...
async function seedUmaValue() {
	...
	try {
		console.log(`🌱 Seeding UMA value (${isRemote ? "remote" : "local"})...`);
		...
		try {
			...
			if (countMatch && parseInt(countMatch[1], 10) > 0) {
				console.log(`⏭️  Active UMA value already exists. Skipping seed.`);
				return;
			}
		} catch {
			// If check fails, continue with seeding
			console.warn(
				"⚠️  Could not check existing UMA values, proceeding with seed...",
			);
		} finally {
			try {
		...
		try {
			...
			console.log(
				`✅ UMA value seeding completed: Created/updated UMA value for ${DEFAULT_UMA_VALUE.year} (${DEFAULT_UMA_VALUE.dailyValue})`,
			);
		} finally {
			// Clean up temp file
			try {
		...
		console.error("❌ Error seeding UMA value:", error);
		throw error;
	}
}
