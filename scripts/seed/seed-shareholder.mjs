 ...
 * Seed Shareholders
 *
 * Generates synthetic shareholder data for dev/preview environments.
...
async function seedShareholders() {
	...
	try {
		console.log(
			`🌱 Seeding shareholders (${isRemote ? "remote" : "local"})...`,
		);

		// Check if shareholders already exist
		const checkSql = "SELECT COUNT(*) as count FROM shareholders;";
		const checkFile = join(
			__dirname,
			`temp-check-shareholders-${Date.now()}.sql`,
		);
		try {
			...
			if (countMatch && parseInt(countMatch[1], 10) > 0) {
				console.log(`⏭️  Shareholders already exist. Skipping seed.`);
				return;
			}
		} catch {
			// If check fails, continue with seeding
			console.warn(
				"⚠️  Could not check existing shareholders, proceeding with seed...",
			);
		} finally {
			try {
		...
		// TODO: Implement shareholder seeding logic
		// Generate synthetic shareholders with realistic data for testing
		// This should include both PERSON and COMPANY entity types with proper relationships
		// For now, skip if no shareholders exist
		console.log(
			"✅ Shareholder seeding completed (no synthetic data generated)",
		);
	} catch (error) {
