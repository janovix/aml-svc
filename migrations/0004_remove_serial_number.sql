-- Migration: Remove serialNumber from transactions
-- Description: Remove serialNumber column from transactions table

-- Note: SQLite doesn't support DROP COLUMN directly, so we need to recreate the transactions table
-- Create new transactions table without serialNumber column
...
-- Copy data from old transactions table (excluding serialNumber)
