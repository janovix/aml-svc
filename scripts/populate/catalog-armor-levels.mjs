#!/usr/bin/env node
/**
 * Populate Armor Levels Catalog (Tipo de Blindaje)
 *
 * This script populates the armor-levels catalog with data from SAT CSV.
...
const CSV_URL = "https://catalogs.janovix.com/armor-levels.csv";
const CATALOG_KEY = "armor-levels";

async function downloadCsv() {
	console.log("📥 Downloading armor-levels.csv...");
...
function parseCsv(csvText) {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		if (values.length >= 2) {
			data.push({
				key: values[0], // Armor level code (usually 1-9)
...
function generateDeterministicId(catalogId, normalizedName) {
	// Create a hash-like ID from catalogId + normalizedName
	...
	// Convert to hex and pad to 32 chars (same length as randomblob(16))
...
function generateSql(catalogId, items) {
	...
	sql.push(`
		...
		VALUES ('${catalogId}', '${CATALOG_KEY}', 'Tipo de Blindaje', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
	`);
	...
	for (const item of items) {
		...
		const metadata = JSON.stringify({
			code: item.key, // Armor level code - used in XML
		}).replace(/'/g, "''");
...
async function populateArmorLevelsCatalog() {
	...
	try {
		console.log(
			`📦 Populating armor-levels catalog (${isRemote ? "remote" : "local"})...`,
		);

		// Download and parse CSV
		const csvText = await downloadCsv();
		const items = parseCsv(csvText);
		console.log(`✅ Parsed ${items.length} armor levels from CSV`);
		...
		const sqlFile = join(__dirname, `temp-armor-levels-${Date.now()}.sql`);
		...
		try {
			...
			console.log("✅ Armor-levels catalog populated successfully!");
		} finally {
			// Clean up temp file
			try {
				unlinkSync(sqlFile);
		...
		console.error("❌ Error populating armor-levels catalog:", error);
		process.exit(1);
	}
}

populateArmorLevelsCatalog().catch((error) => {
