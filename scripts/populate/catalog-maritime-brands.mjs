#!/usr/bin/env node
/**
 * Populate Maritime Brands Catalog
 *
 * This script populates the maritime-brands catalog with real data.
...
// Path to the SQL population file
const sqlFile = join(__dirname, "../seed-maritime-brands.sql");
...
try {
	console.log(
		`📦 Populating maritime-brands catalog (${isRemote ? "remote" : "local"})...`,
	);
	...
	console.log("✅ Maritime-brands catalog populated successfully!");
} catch (error) {
	console.error("❌ Error populating maritime-brands catalog:", error);
	process.exit(1);
}
