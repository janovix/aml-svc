...
describe("Alerts API", () => {
	...
	const testOrgId = "alerts-test-org";
	...
	describe("POST /api/v1/alerts/:id/cancel", () => {
		it("returns 404 for non-existent alert", async () => {
			const res = await SELF.fetch(
				"http://local.test/api/v1/alerts/non-existent-alert/cancel",
				{
					method: "POST",
					headers: {
		...
		it("returns 400 when reason is missing in request body", async () => {
			const res = await SELF.fetch(
				"http://local.test/api/v1/alerts/some-alert-id/cancel",
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${testToken}`,
