#!/usr/bin/env node
/**
 * Populate Vulnerable Activities Catalog
 *
 * This script populates the vulnerable-activities catalog with data from SAT CSV.
...
const CSV_URL = "https://catalogs.janovix.com/vulnerable-activities.csv";
const CATALOG_KEY = "vulnerable-activities";

async function downloadCsv() {
	console.log("📥 Downloading vulnerable-activities.csv...");
...
function parseCsv(csvText) {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		if (values.length >= 2) {
			data.push({
				clave: values[0], // e.g., "VEH", "JYS"
				nombre: values[1] || "", // Full name
				nombreCorto: values[2] || "", // Short name
				descripcion: values[3] || "", // Description
			});
		}
	}
...
function generateSql(catalogId, items) {
	...
	sql.push(`
		...
		VALUES ('${catalogId}', '${CATALOG_KEY}', 'Vulnerable Activities', 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
	`);
...
async function populateVulnerableActivitiesCatalog() {
	...
	try {
		console.log(
			`📦 Populating vulnerable-activities catalog (${isRemote ? "remote" : "local"})...`,
		);

		// Download and parse CSV
		const csvText = await downloadCsv();
		const items = parseCsv(csvText);
		console.log(`✅ Parsed ${items.length} vulnerable activities from CSV`);
		...
		const sqlFile = join(
			__dirname,
			`temp-vulnerable-activities-${Date.now()}.sql`,
		);
		...
		try {
			...
			console.log("✅ Vulnerable-activities catalog populated successfully!");
		} finally {
			// Clean up temp file
			try {
				unlinkSync(sqlFile);
		...
		console.error("❌ Error populating vulnerable-activities catalog:", error);
		process.exit(1);
	}
}

populateVulnerableActivitiesCatalog().catch((error) => {
