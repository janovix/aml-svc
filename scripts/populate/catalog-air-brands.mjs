#!/usr/bin/env node
/**
 * Populate Air Brands Catalog
 *
 * This script populates the air-brands catalog with real data.
...
const sqlFile = join(__dirname, "../seed-air-brands.sql");
...
try {
	console.log(
		`📦 Populating air-brands catalog (${isRemote ? "remote" : "local"})...`,
	);
	...
	console.log("✅ Air-brands catalog populated successfully!");
} catch (error) {
	console.error("❌ Error populating air-brands catalog:", error);
	process.exit(1);
}
